...
/**
 *   A struct to pass parameters for tone-mapping.
 */
typedef struct LadybugToneMappingParams
{
    ...
    * This value must be larger or equal to 0. If 0, the local average
 ...
 *   configured. If any un-supported image types are specified by
 *   uiImageTypes, it returns LADYBUG_INVALID_ARGUMENT.
 *
 * Remarks:
 *
 ...
 *
 * Example:
 *   If the application needs to display panoramic images on the
 ...
 *
 * Remarks:
 *   If pRenderingInfo is NULL, this function returns LADYBUG_INVALID_ARGUMENT.
 *
 */
LADYBUGDLL_API LadybugError 
 ...
 *
 * Example:
 *   If the raw image size is 1024x768 and the application needs
 ...
 *   ladybugUpdateTextures() are 
 *   processed to minimize sampling artifacts that may appear on the rendered
 *   images.
 ...
 * 
 * Example:
 *   To enable/disable anti-aliasing, call these Ladybug API functions in the
 *   following order:
 *   ladybugSetAntiAliasing( true/false );
 *   ladybugUpdateTextures();
 *   ladybugDisplayImage() or ladybugRenderOffScreenImage();
 *
 * @see   ladybugUpdateTextures(),
 *   ladybugDisplayImage(),
 *   ladybugRenderOffScreenImage()
 *
 */
LADYBUGDLL_API LadybugError 
ladybugSetAntiAliasing( LadybugContext context,
                           bool bEnable );
 ...
 * @return   A LadybugError indicating the success of the function.
 *
 * Remarks: This function is used to initialize the Ladybug library and the 
...
/** Name: ladybugGetOffScreenImageSize()
 *
 ...
 *                 This would be either LADYBUG_BGRU or LADYBUG_BGR for
 *                 low dynamic range rendering.
 *                 For high dynamic range rendering, LADYBUG_BGR32F is recommended.
 ...
 * 
 * Example:
 *   If the width of the texture buffer is 1024, and the width
 ...
 *
 *   NOTE:  This function cannot be used with Ladybug3 cameras.
 *
 * @see   ladybugConfigureOutputImages(),
 *   ladybugRenderOffScreenImage(),
 *   ladybugSetOffScreenImageSize(),
 ...
 *   are taken with different exposure settings, so the level of image intensity may appear different across
 *   the borders of the stitched image. By enabling intensity adjustment,
 ...
 *
 * @param   context - The LadybugContext to access.
 * @param  bEnable - Specifies whether to enable this functionality. The default value
 *             is off.
 *
 * @return   A LadybugError indicating the success of the function.
 *
 * Remarks:
 *   This functionality requires OpenGL version 2.0 or later.
 ...
 */
LADYBUGDLL_API LadybugError
ladybugSetTextureIntensityAdjustment(  LadybugContext		context,
		                       bool				bEnable);


/**
 *
 *   This function enables tone mapping in image rendering.
 *   Tone mapping compresses high dynamic range images to make them appear more pleasing to 
 *   the human eye. The conversion used for the mapping is based on Reinhard's
 ...
 *   This functionality requires OpenGL version 2.0 or later. 
 *
 *   Testing indicates that tone mapping may produce unexpected results 
 *   on ATI graphics cards that support OpenGL version 3.0 or earlier. 
 *
 * @see   ladybugSetToneMappingParams()
 *
 */
LADYBUGDLL_API LadybugError
ladybugSetToneMapping(  LadybugContext		context,
		           bool				bEnable);

/**
 *
 *   This function changes the processing parameters for tone mapping.
 *
 * @see   LadybugToneMappingParams
 *   ladybugSetToneMapping()
 *
 */
LADYBUGDLL_API LadybugError
ladybugSetToneMappingParams(  LadybugContext		context,
