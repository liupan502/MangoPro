...
/**
 *   An "image" of 3D points corresponding to values of the
 *   source image.
 */
typedef struct LadybugImage3d
{
   ...
   /** Cylindrical height minimum */
   float fCylHeightMin;

   /** Cylindrical height maximum */
   float fCylHeightMax;

   /** The area for the pixel data. */
   LadybugPoint3d*  ppoints; 
   
} LadybugImage3d;


/**
 *   This enumeration describes available viewing angle options for 
 *   panoramic and spherical image rendering.
 *
 * Remarks:
...
typedef enum LadybugPanoAngle
{
   ...
   LADYBUG_FRONT_0_POLE_5, // front camera = 0, pole camera = 5
   LADYBUG_FRONT_1_POLE_5, // front camera = 1, pole camera = 5
   LADYBUG_FRONT_2_POLE_5, // front camera = 2, pole camera = 5
   LADYBUG_FRONT_3_POLE_5, // front camera = 3, pole camera = 5
   LADYBUG_FRONT_4_POLE_5, // front camera = 4, pole camera = 5
   LADYBUG_FRONT_5_POLE_0, // front camera = 5, pole camera = 0
   LADYBUG_FRONT_5_POLE_1, // front camera = 5, pole camera = 1
   LADYBUG_FRONT_5_POLE_2, // front camera = 5, pole camera = 2
   LADYBUG_FRONT_5_POLE_3, // front camera = 5, pole camera = 3
   LADYBUG_FRONT_5_POLE_4, // front camera = 5, pole camera = 4
...
/**
 *   This enumeration describes the available panoramic image mapping types.
 *
 * Remarks:
 ...
 *   rectification are performed. 
 *   Also, this function should be called before beginning to transfer
 *   images with ladybugStart(), ladybugStartLockNext(), etc.
 *
 ...
 *
 * @param   context        - The LadybugContext to access.
 * @param  pPanoViewAngle - Location to return the viewing angle. See LadybugPanoAngle.
 *
 * @return   A LadybugError indicating the success of the function.   
 *
 ...
 *   stitched.
 *   By default, it is 20m.
 *   This function reinitializes the alpha masks if they 
 *   have already been initialized.
 *   
 * @see   http://www.ptgrey.com/support/kb/index.asp?a=4&q=250
 ...
 *   performed within small, localized areas, so that different stitching distances can be applied to different areas. Localized stitching is achieved by searching for the distance that best overlaps 
 *   the images in each area.
 *   If dynamic stitching is not applied, stitching is performed according to a fixed distance 
 *   for the entire sphere.
 *   Dynamic stitching is recommended when subjects in the scene are located within
 *   various distances and prominent stitching errors cannot be rectified
 *   using a fixed stitching distance.
 *   Note that dynamic stitching is time consuming
 *   and may result in unexpected effects when a search for the best
 *   distance fails.
 *
 * @param   context     - The LadybugContext to access.
 ...
 *                 applied. This parameter is useful when dynamic stitching should
 *                 only be applied to a certain location in the image.
 *                 The fX, fY and fZ members of LadybugPoint3d struct must be 
 *                 set. Other members are ignored.
 ...
 *   http://www.ptgrey.com/support/kb/index.asp?a=4&q=250
 *
 */
LADYBUGDLL_API LadybugError 
 ...
 *   Also, it is assumed that the aspect ratio of the rectified image is
 ...
 *
 * @param   context    - The LadybugContext to access.
 * @param  pPoints    - The 4 points on the two lines to which the Z axis should be
 *                aligned.
 ...
 *   Also, the lines should not be too close to each other or too 
 *   close to an exact opposite position (180 degrees apart).
 *
 * @see   ladybugSet3dMapRotation()
 * 
 */
 ...
 *   Alpha mask changes as the blending width changes or the stitching sphere size 
 ...
 *
 * @see   ladybugSetAlphaMaskPathname()
 ...
 *   
 *   If the pathname is set, Ladybug uses it to save alpha mask 
 *   files. Otherwise, the current directory is used.       
 *
 * @see   ladybugInitializeAlphaMasks()
 *   ladybugLoadAlphaMasks();
 *
 */
LADYBUGDLL_API LadybugError
 ...
 * @return   A LadybugError indicating the success of the function.
 *
 *   LADYBUG_OK is returned if the alpha map was loaded successfully.
 *
 *   LADYBUG_COULD_NOT_OPEN_FILE is returned if the alpha mask could not
 *   be located.     
 *
 * Remarks:
 ...
 *   As long as you use the same color image buffers, you do not have to set
 ...
 *
 * Remarks:
 *   Set to "false" if you are not doing any texture map blending.  
